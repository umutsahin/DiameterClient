...
import com.optiva.charging.openapi.diameter.common.enumeration.AvpCodeTable;
import io.netty.buffer.ByteBuf;

import java.nio.ByteBuffer;

public class AvpHeader {
    private AvpCode avpCode;
    ...
    AvpHeader(ByteBuffer buffer) {
        ...
        avpCode = AvpCodeTable.get(vendorId, avp);
        avpValueLength -= 8;
        avpHeaderLength = avpLength - avpValueLength;
    }

    AvpHeader(ByteBuf buffer) {
        final int avp = buffer.readInt();
        final int flagsAndLength = buffer.readInt();
        isVendorSpecific = (flagsAndLength >>> 31) == 1;
        isMandatory = ((flagsAndLength >>> 30) & 0x01) == 1;
        isProtected = ((flagsAndLength >>> 29) & 0x01) == 1;
        avpLength = flagsAndLength & 0x00FFFFFF;
    ...
    AvpHeader adjust() {
        avpHeaderLength = isVendorSpecific
                          ? 12
                          : 8;
    ...
    */
    ...
    public void convertToBytes(ByteBuf buffer, int bytesWritten) {
        buffer.writeInt(avpCode.getCode());
        int flags = ((((isVendorSpecific
                        ? 1
                        : 0) * 2) + (isMandatory
                                     ? 1
                                     : 0)) * 2 + (isProtected
                                                  ...
                                                  : 0)) * 536870912;
        avpLength = avpHeaderLength + bytesWritten;
        buffer.writeInt(flags | avpLength);
        if (isVendorSpecific) {
            buffer.writeInt(vendorId);
        }
    }

    @Override
    public String toString() {
        return "AvpHeader{" + "avpCode=" + avpCode + '}';
    }
    ...
    void setAvpCode(AvpCode avpCode) {
    ...
    public boolean equals(Object o) {
        ...
        AvpHeader that = (AvpHeader) o;

        if (avpLength != that.avpLength) {
            return false;
        }
        ...
        if (isVendorSpecific != that.isVendorSpecific) {
            return false;
        }
        if (vendorId != that.vendorId) {
            return false;
        }
        if (avpValueLength != that.avpValueLength) {
            return false;
        }
        if (avpHeaderLength != that.avpHeaderLength) {
            return false;
        }
        return avpCode != null
               ? avpCode.equals(that.avpCode)
               : that.avpCode == null;
    }

    @Override
    public int hashCode() {
        int result = avpCode != null
                     ? avpCode.hashCode()
                     : 0;
        result = 31 * result + avpLength;
        result = 31 * result + (isMandatory
                                ? 1
                                : 0);
        result = 31 * result + (isProtected
                                ? 1
                                : 0);
        result = 31 * result + (isVendorSpecific
                                ? 1
                                : 0);
